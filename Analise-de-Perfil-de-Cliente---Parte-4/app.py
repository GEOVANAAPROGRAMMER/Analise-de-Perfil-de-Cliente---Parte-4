...
numeric_features = ['Idade', 'Finalidade Profissional',
                    'Finalidade Acadêmica', 'Finalidade Entreterimento']
...
def extrair_valores(faixa_salarial):
    ...
    if faixa_salarial.startswith('menos de'):
        ...
        if len(partes) == 2:
            ...
            raise ValueError(f"Formato desconhecido de faixa salarial: {
                             faixa_salarial}")
...
def recomendar_ofertas(faixa_salarial_cliente, ofertas_df):
    try:
        minimo_cliente, maximo_cliente = extrair_valores(
            faixa_salarial_cliente)
    ...
    for _, row in ofertas_df.iterrows():
        try:
            minimo_oferta, maximo_oferta = extrair_valores(
                row['Faixa Salarial'])
...
def dashboard():
    ...
    try:
        feature_names = numeric_features + list(modelo.named_steps['preprocessor'].transformers_[
                                                1][1].named_steps['onehot'].get_feature_names_out(categorical_features))
        ...
        ax.barh(top_features['Feature'],
                top_features['Importance'], color='skyblue')
    ...
    try:
        fig, ax = plt.subplots(figsize=(10, 5))
        compras_por_estado = perfil_df[perfil_df['Comprou']
                                       == 1]['Estado onde mora'].value_counts()
        ax.plot(compras_por_estado.index, compras_por_estado.values,
    ...
    try:
        ...
        values = [finalidade_profissional,
                  finalidade_academica, finalidade_entretenimento]
        labels = ['Finalidade Profissional',
                  'Finalidade Acadêmica', 'Finalidade Entretenimento']
...
def add_client():
    if request.method == 'POST':
        try:
            ...
            finalidade_profissional = int(
                request.form.get('purpose_professional', 0))
            finalidade_academica = int(request.form.get('purpose_academic', 0))
            finalidade_entretenimento = int(
                request.form.get('purpose_entertainment', 0))
            ...
            cliente_preprocessado = modelo.named_steps['preprocessor'].transform(
                client_data)

            # Prever a probabilidade de compra
            probabilidade_compra = modelo.named_steps['classifier'].predict_proba(
                cliente_preprocessado)[0][1]
            ...
            if not similaridade.empty:
                perfis_similares = similaridade.describe(
                    include='all').to_dict()
                mensagem_perfis = f"Perfis de clientes que compraram produtos semelhantes: {
                    perfis_similares}."
            else:
            ...
            mensagem_chatgpt = (
                ...
                f"Escreva em 3 linhas a resposta e a oferta deve estar em um formato diferente, cada frase em linhas separadas."
            )
